...
def test_update_student():
    response = client.put("/students/0", json={"first_name": "Jane", "last_name": "Doe"})
    assert response.status_code == 200
    assert response.json() == {"first_name": "Jane", "last_name": "Doe"}
    
def test_update_student_not_found():
    response = client.put("/students/100", json={"first_name": "Jane", "last_name": "Doe"})
    assert response.status_code == 404
    assert response.json() == {"detail": "Student not found"}
...
def test_add_mark():
    response = client.post("/students/0/marks/5")
    assert response.status_code == 200
    assert response.json() == {"message": "Mark added"}

def test_add_mark_not_found():
    response = client.post("/students/100/marks/5")
    assert response.status_code == 404
    assert response.json() == {"detail": "Student not found"}

def test_get_marks():
    response = client.get("/students/0/marks")
    assert response.status_code == 200
    assert response.json() == [5.0, 4.5, 4.0, 3.5, 3.0, 2.0]
